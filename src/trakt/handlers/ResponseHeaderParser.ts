...
export class ResponseHeaderParser {
    ...
    parseResponseHeaders(responseHeaders: IDictionary, traktResponse: TraktResponse<any>) {
        ...
        traktResponse.startDate = new Date(responseHeaders[this.HEADER_STARTDATE_KEY]) ?? null;
        traktResponse.endDate = new Date(responseHeaders[this.HEADER_ENDDATE_KEY]) ?? null;
